...
The most up-to date version of this document is available at:
    ...
    https://github.com/Marcdnd/electrum-server-cesc/blob/master/HOWTO.md
...
document, we assume that user is called 'cescuser'). We also assume the
cescuser user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run cryptoescudod and keep a copy of the full blockchain,
which is roughly 6 GB (February 2016). Ideally you have a machine with 4 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit cryptoescudod 
to 8 concurrent connections by disabling incoming connections. electrum-server-cesc may
...
CPU speed is less important than fast I/O speed. electrum-server-cesc makes uses of one core 
only leaving spare cycles for cryptoescudod. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running cryptoescudod and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `cryptoescudod` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser cescuser --disabled-password
    $ sudo apt-get install git
    $ sudo su - cescuser
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/cescuser/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download cryptoescudo

We currently recommend cryptoescudod v1.2.0.1-20160331 stable.

Download here: http://cryptoescudo.pt/download/01020001/linux/cryptoescudo-1.2.0.1-linux.zip

### Step 3. Configure and start cryptoescudod

In order to allow Electrum to "talk" to `cryptoescudod`, we need to set up an RPC
username and password for `cryptoescudod`. We will then start `cryptoescudod` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.cryptoescudo
    $ $EDITOR ~/.cryptoescudo/cryptoescudo.conf

Write this in `cryptoescudo.conf`:
...
If you have an existing installation of cryptoescudod and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ cryptoescudod -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `cryptoescudod`:

    $ cryptoescudod

Allow some time to pass, so `cryptoescudod` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ cryptoescudod getinfo

Before starting the Electrum server your cryptoescudod should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start cryptoescudd at boot
time, running as the 'cescuser' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/Marcdnd/electrum-server-cesc.git
    $ cd electrum-server-cesc
...
If you have the time and nerve, you can import the blockchain yourself.

As of August 2016 it takes between one and two days to import 500k blocks, depending
...
Electrum reads a config file (/etc/electrum-cesc.conf) when starting up. This
file includes the database setup, cryptoescudod RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-cesc.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-server-cesc.
If you're using the user `cescuser` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "cescuser hard nofile 65536" >> /etc/security/limits.conf
     echo "cescuser soft nofile 65536" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u cescuser -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close cryptoescudod for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting cryptoescudod (together with electrum-server-cesc) on a weekly basis to clear out unconfirmed
...
Note: If you want to run the server without installing it on your system, just run 'run_electrum_server_cesc" as the
unprivileged user.
...
If your system supports it, you may add electrum-server-cesc to the /etc/init.d directory.
...
cryptoescudos to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-cesc and we'll try to congratulate you
on supporting the community by running an Electrum-CESC node.
