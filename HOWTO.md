...
make sure you limit cryptoescudod to 8 concurrent connections. If you have more 
...
### Step 2. Download cryptoescudod

Older versions of Electrum used to require a patched version of cryptoescudod. 
This is not the case anymore since cryptoescudod supports the 'txindex' option.
We currently recommend cryptoescudod 1.5.1 stable.

If your package manager does not supply a recent cryptoescudod or you prefer to compile it yourself,
here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config git
    $ sudo su - cescuser
    $ mkdir {src,bin}
    $ cd ~/src &&git clone https://github.com/Marcdnd/cryptoescudo.git
    $ cd cryptoescudo/src
    $ ./configure 
    $ make
    $ strip src/cryptoescudod 
    $ cp -a src/cryptoescudod ~/bin
...
Allow some time to pass for `cryptoescudod` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ cryptoescudo getblocktemplate
...
You should also set up your system to automatically start cryptoescudod at boot
...
### For cryptoescudo this step need revision
...
If you're using user cescuser to run electrum and have added it manually like shown in this HOWTO run 
...
1. To increase security you may want to close cryptoescudod for incoming connections and connect outbound only
